...
void main() {
  ...
  vec3 textureNormal = texture(uTexNormal, vTexCoord).xyz * 2.0 - 1.0;

  vec3 normal = normalize(vNTMat * textureNormal);

  vec3 viewDir = normalize(-vEyePos);
  vec3 lightDir = normalize(uLight - vEyePos);
  vec3 lightDir2 = normalize(uLight2 - vEyePos);

  float nDotL = dot(normal, lightDir); // shading
  ...
  nDotL = dot(normal, lightDir2); // shading

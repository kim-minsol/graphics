...
inline Quat log(const Quat& q) {
    ...
    double q_norm = std::sqrt(norm2(q));
    
    if (v_norm < CS380_EPS2) {
        return Quat(std::log(q_norm), 0, 0, 0);
    } else {
        double theta = std::atan2(v_norm, w);
        Cvec3 v_unit = v / v_norm;
        return Quat(std::log(q_norm), v_unit * theta);
    }
}
...
inline Quat exp(const Quat& q) {
    ...
    if (v_norm < CS380_EPS2) {
        return Quat(e_w, 0, 0, 0);
    } else {
        Cvec3 v_unit = v / v_norm;
        return Quat(e_w * std::cos(v_norm), v_unit * (e_w * std::sin(v_norm)));
    }
}

inline Quat pow(const Quat& q, double t) {
    return exp(log(q) * t);
}

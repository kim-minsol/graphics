...
static int g_width             = 256;       // screen width
...
struct ShaderState {
  ...
  ShaderState(const char* vsfn, const char* fsfn) {
    ...
    h_uXCoefficient = safe_glGetUniformLocation(h, "uXCoefficient");
    h_uYCoefficient = safe_glGetUniformLocation(h, "uYCoefficient");
...
struct SquareGeometry {
  ...
  SquareGeometry() {
    static GLfloat sqPos[12] = { // two triangles, each triangle with three verticies
      -.5, -.5,
      .5,  .5,
      .5,  -.5,
    ...
    static GLfloat sqCol[18] =  { // two triangles with RGB values
      1, 0, 0,
      0, 1, 1,
      0, 0, 1,
...
static void display(void) {
  ...
  if (g_width > g_height) {
    XCoef = (float) g_height / (float) g_width;
    YCoef = 1.;
  }
  else {
    XCoef = 1.;
    YCoef = (float) g_width / (float) g_height;
  }

  safe_glUniform1i(curSS.h_uTexUnit0, 0);
  safe_glUniform1i(curSS.h_uTexUnit1, 1);
  safe_glUniform1f(curSS.h_uXCoefficient, XCoef);
  safe_glUniform1f(curSS.h_uYCoefficient, YCoef);
...
// setup the window and callback functions
static void initGlutState(int argc, char **argv) {
  glutInit(&argc,argv);
  glutInitDisplayMode(GLUT_RGBA|GLUT_DOUBLE|GLUT_DEPTH); // make pixels with RGB, alphas, double buffering
...
// setup the background color for the window
